...
function navigateToFolder(folderId, folderName) {
    ...
    // Update the current folder in the stack if it exists, otherwise add it
    const existingFolderIndex = state.folderStack.findIndex(f => f.id === folderId);
    if (existingFolderIndex >= 0) {
        // Update the existing folder name in case it changed
        state.folderStack[existingFolderIndex].name = folderName;
        // Keep only up to this folder in the stack
        state.folderStack = state.folderStack.slice(0, existingFolderIndex + 1);
    } else {
        // Add new folder to the stack
...
async function updateSuggestions(prefix = '') {
    ...
    try {
        ...
        // Clear the suggestion box but keep the header if it exists
        const existingHeader = dom.suggestionBox.querySelector('div:first-child');
        dom.suggestionBox.innerHTML = '';

        // Always show the folder header when in a folder
        if (state.currentFolderId && state.folderStack.length > 0) {
            const header = document.createElement('div');
            header.className = 'p-2 bg-gray-100 text-black text-sm font-medium flex items-center cursor-pointer hover:bg-gray-200 transition-colors';
            header.innerHTML = `
                <div class="flex items-center w-full">
            ...
            // Make the header clickable for navigation
            header.addEventListener('click', (e) => {
                e.stopPropagation();
                navigateToParentFolder();
            });
        ...
        // Check if this is a 404 error (folder not found or empty)
        if (error.response && error.response.status === 404) {
            // Don't show any additional message for empty folders
            return;
        }

        // For other errors, show the error message below the header
        const errorDiv = document.createElement('div');
        errorDiv.className = 'p-4 text-center';
        errorDiv.innerHTML = `
            <div class="text-black-700">Không có nội dung</div>
        `;
        dom.suggestionBox.appendChild(errorDiv);
        dom.suggestionBox.classList.remove('hidden');
    }
}
        ...
        content.innerHTML = `
            <div class="space-y-4 text-gray-200">
                <div class="pb-2 border-b border-gray-700">
                    <p class="text-xs font-semibold text-black  uppercase tracking-wider">Tên file</p>
                    <p class="mt-1 text-black">${original_file_name || 'N/A'}</p>
                </div>
                 <div class="pb-2">
                    <p class="text-xs font-semibold text-black  uppercase tracking-wider">Nội dung trích xuất</p>
...
// Xử lý sự kiện storage để cập nhật khi có file mới được upload
async function handleStorageEvent(event) {
    if (event.key === 'file_uploaded' && event.newValue) {
        console.log('Phát hiện file mới được upload, đang làm mới quyền truy cập...');
        try {
            await refreshAccess();
            // Load lại danh sách từ khóa sau khi refresh token
            await loadKeywords();
            console.log('Đã cập nhật quyền truy cập thành công');
        } catch (error) {
            console.error('Lỗi khi làm mới quyền truy cập:', error);
        }
    }
...
async function initializeApp() {
    ...
    // Thêm sự kiện beforeunload để refresh token trước khi tải lại trang
    window.addEventListener('beforeunload', async (e) => {
        try {
            // Sử dụng sendBeacon để đảm bảo request được gửi đi trước khi trang đóng
            const data = new FormData();
            data.append('token', state.accessToken);
            navigator.sendBeacon(`${API_BASE_URL}/file/me/refresh-access`, data);
        } catch (error) {
            console.error('Lỗi khi làm mới token trước khi đóng trang:', error);
        }
    });
    try {
